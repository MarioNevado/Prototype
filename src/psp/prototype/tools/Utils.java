...
import psp.prototype.factories.AsimmetricKeyFactory;
import psp.prototype.factories.SimmetricKeyFactory;
...
public class Utils {
    ...
    public static PublicKey getPublicKey(String route) throws IOException {
        try (ObjectInputStream oos = new ObjectInputStream(new FileInputStream(route))) {
            ...
            throw new IOException("ERROR. El fichero de la clave Pública no es válido.");
        }
    }

    public static PrivateKey getPrivateKey(String route) throws IOException{
        try (ObjectInputStream oos = new ObjectInputStream(new FileInputStream(route))) {
            AsimmetricKeyFactory key = (AsimmetricKeyFactory) oos.readObject();
            return key.getPrivateKey();
        } catch (IOException | ClassNotFoundException ex) {
            throw new IOException("ERROR. El fichero de la clave Privada no es válido");
        }
    }
